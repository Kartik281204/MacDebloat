...
The bash script is the reference implementation, but the same 30 tweaks are also available through:

- **[`deblaot/`](deblaot/)** — a FastAPI backend exposing them over a local REST API, with structured (SQLite-backed) history/revert instead of a text log.
- **[`deblaot-gui/`](deblaot-gui/)** — a PyQt5 desktop app for it.
- **The mobile web UI `deblaot` itself hosts** at `/` — open it in Mobile Safari (works great on an iPhone 15) and add it to your Home Screen for an app-like icon. Reachable from an iPhone over Wi-Fi with one extra step (`DEBLAOT_HOST=0.0.0.0 ./run.sh`), token-protected so only this Mac and whoever has that token can reach it — see `deblaot/README.md`.

All four talk to the exact same tweak catalog; none of them duplicate the underlying `defaults`/`pmset` logic.
